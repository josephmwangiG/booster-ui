        ...
        <div class="title">
          ...
          <span class="text-gray-500 text-sm">
            ...
            <span class="font-semibold text-gray-700">
              {{ storesStore?.stores?.length?.toLocaleString() || 0 }}
            </span>
            stores
          </span>
        </div>
      ...
      <div class="overflow-x-auto w-full">
        <table ref="dataTableRef" class="w-full border-collapse text-sm">
          <thead class="bg-gradient-to-r from-blue-50 to-indigo-50">
            <tr>
              <th class="t-th text-left">Store Name</th>
              <th class="t-th text-left">Description</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr
              v-for="(item, index) in (storesStore.stores as any)"
              :key="index"
              :class="index % 2 != 0 ? 'bg-gray-50' : ''"
              class="hover:bg-blue-50/40 transition-colors duration-300"
            >
              <td class="t-td font-medium text-gray-700">{{ item.store_name || item.name }}</td>
              <td class="t-td">{{ item.description || item.short_description }}</td>
              ...
              <td class="t-td text-center">
                <div class="flex justify-center space-x-2">
                  <button @click="editStore(item)" class="bg-blue-100 text-blue-700 hover:bg-blue-200 text-xs px-3 py-1 rounded-lg shadow-sm transition duration-300">
                    Edit
                  </button>
                  <button @click="deleteStore(item.id)" class="bg-red-100 text-red-700 hover:bg-red-200 text-xs px-3 py-1 rounded-lg shadow-sm transition duration-300">
                    Delete
                  </button>
                </div>
              </td>
...
const storesStore = useAdmStoresStore();

const activeModal = ref(false);
const action = ref('create');
const formData = ref({});

const editStore = (storeItem: any) => {
  action.value = 'edit';
  formData.value = { ...storeItem };
...
const handleFormSubmit = () => {
  // Refresh the stores list after form submission
