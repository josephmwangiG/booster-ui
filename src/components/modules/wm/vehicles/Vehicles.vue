  ...
  <div class="content">
    <div class="top-section">
      <div class="bread-crumb">
        ...
        <span class="text-sm">
          <span class="text-gray-400">Home ></span> Vehicles
        </span>
      </div>
    </div>
    <div class="">
      <div class="w-full bg-white p-3 lg:p-6 mt-3 lg:mt-6">
        ...
        <div class="grid grid-cols-3 mt-3 gap-6">
          <div class="border border-dashed p-3 px-4 rounded col-span-1">
            <h2 class="font-semibold">{{ store.vehicles.length.toLocaleString() }}</h2>
            <span class="text-gray-400 text-sm">Vehicles</span>
          </div>
      ...
      <template #header>
        <div class="modal-header flex justify-between items-center">
          <h3 class="text-base font-semibold leading-6 text-gray-900" id="modal-title">
            {{ action == "create" ? "Add" : "Edit" }} Vehicle
          </h3>
          <CloseBtnComponent @click="dialogVisible = false" />
        </div>
      </template>
      <VehicleFormModal @close-modal="dialogVisible = false" :form="formData" :action="action"
        :key="(dialogVisible as any)"></VehicleFormModal>
    </el-dialog>
  </teleport>
</template>
