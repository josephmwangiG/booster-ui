  ...
  <div class="mt-2" v-if="!loading">
    ...
    >
      ...
      <el-form-item prop="unit_id" class="flex-1" :label="'Unit'">
        <el-select
          @change="getRentAmount"
          v-model="formData.unit_id"
          placeholder="Select a unit"
          :disabled="action === 'edit'"
        >
          <el-option
            v-for="item in unit_items"
            :key="item.id"
...
const unit_items = computed(() => {
  if (formData.property_id) {
    return store.properties
      .find((item: any) => item.id == formData.property_id)
      ?.units.filter(
        (item: any) => item.available || item.id == formData.unit_id
      );
  } else {
    return [];
  }
...
const getRentAmount = (_e: any) => {
  if (formData.unit_id) {
    formData.rent_amount = unit_items.value.find(
      (item: any) => item.id == formData.unit_id
    )?.rent;
  } else {
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    if (props.action === "edit") {
      const res = await store.updateTenancy(formData);
      if (res.status == 200 || res.status == 201) {
        resetForm(itemFormRef.value as FormInstance);
        ElNotification({
          title: "Success",
          message: "Tenancy was updated",
          type: "success",
        });
        emits("close-modal");
      }
