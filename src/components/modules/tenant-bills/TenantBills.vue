<template>
    ...
    <div class="">
      ...
      <div class="space-y-6 p-3 lg:p-6 mt-3 lg:mt-6 bg-white col-span-3">
        <div class="flex justify-between align-center">
          <div class="">
            ...
            <span class="text-gray-400 text-sm">
              {{ (filteredTenantBills || []).length }} of
              {{ (store.tenantBills || []).length }} items found
            </span>
          </div>
          <div class="actions flex gap-2 my-auto justify-end">
            ...
            <button
              @click="navigateToGenerateBills"
              class="btn-primary my-auto mt-1 h-[35px]"
            >
              Generate Bills
            </button>
          </div>
        ...
        <div class="overflow-x-auto w-full">
          <div
            v-if="!filteredTenantBills || filteredTenantBills.length === 0"
          ...
          <table v-else class="w-full" ref="dataTableRef">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr
                v-for="(item, index) in filteredTenantBills"
                :key="index"
                      ...
                      <template #dropdown>
                        <el-dropdown-item>
                          <router-link
                            ...
                            ><i
                              class="ri-external-link-line text-orange-500"
                            ></i>
                            View</router-link
                          >
                        </el-dropdown-item>
  ...
  <teleport to="body">
    ...
    <el-dialog
      v-model="utilityDetailsVisible"
      :show-close="false"
      width="50%"
      class="utility-details-modal"
    >
      <template #header>
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span class="ml-2 font-semibold">{{
                selectedUtilityBill.tenant_name
              }}</span>
            </div>
            <div>
              <span class="font-medium text-gray-600">Period:</span>
              <span class="ml-2 font-semibold">{{
                selectedUtilityBill.period
              }}</span>
            </div>
            <div>
              <span class="font-medium text-gray-600">Total Amount:</span>
              <span class="ml-2 font-semibold"
                >KES {{ selectedUtilityBill.amount?.toLocaleString() }}</span
              >
            </div>
            <div>
              ...
              <span
                class="ml-2"
                :class="
                  selectedUtilityBill.payment_status === 'Paid'
                    ? 'text-green-600'
                    : 'text-orange-600'
                "
              >
            ...
            <table class="w-full text-sm">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="text-right p-2 font-medium text-gray-700">
                    Amount
                  </th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200">
                <tr
                  v-for="item in getBillUtilities(selectedUtilityBill.id)"
                  :key="item.id"
...
const filteredTenantBills = computed(() => {
  ...
  if (selectedStatus.value) {
    filtered = filtered.filter(
      (bill: any) => bill.payment_status === selectedStatus.value
    );
  }
...
const getBillUtilities = (billId: string) => {
  ...
  const utilities = store.tenantBillItems.filter(
    (item) => item.tenant_bill_id === billId && item.item_name !== "Rent"
  );
...
onMounted(async () => {
  ...
  try {
    ...
    if (
      dataTableRef.value &&
      filteredTenantBills.value &&
      filteredTenantBills.value.length > 0
    ) {
