...
export const useAdmUsersStore = defineStore("adm-users", {
  ...
  actions: {
    async getUsers() {
      const res = await axios.get("/api/app/adm/users", this.headers);
      this.users = res.data;
    },
    ...
    async getOrganizations() {
      const res = await axios.get("/api/app/adm/organizations", this.headers);
      this.organizations = res.data;
    },
    async getOrganization(id: string) {
      const res = await axios.get("/api/app/adm/organizations/" + id, this.headers);
      this.organization = res.data.organization;
      this.modules = res.data.modules;
    },
    async updateOrganization(data: any) {
      try {
        const res = await axios.put("/api/app/adm/organizations/" + data.id, data, {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
        });
